...
(player2(_), \+deck(_) -> player2(XP2), fixvalue(XP2,[card(AC1,AV1), card(AC2,AV2)]); AC1='*', AV1='*', AC2='*', AV2 ='*'), %LÄGG TILL \+deck(_)  som ett argument i ifen för att bara visa vid whowon!
...
format('        ##~t~w~36|~t~d$    ##~55|~n', ['---   ---',Stack]),  %~t = tab, ~w =  nästa arguments typ, ~43| =  ska vara en brytpunkt, ~n = newline
format('          ~`#t~53|~n', []).
